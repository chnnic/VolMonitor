...
当前版本:`VolMon.sh` v1.4.22;`volmon-node.sh` v1.1.0。
...
- **名称**:唯一标识,只允许字母、数字、点、下划线和短横线,用于状态文件和日报基线;需要改显示名时优先改“备注”。
...
- 节点管理里需要选择节点时,输入节点名称或列表序号都可以;如果节点名本身就是纯数字,脚本会优先按名称匹配。
...
- `x` 关闭
- `t` 立即发送一次
- 节点管理里可执行 `u` 重置流量基准,设置每月固定重置日与起算时已用的上下行流量;也可执行 `c` 修改当前周期已用,脚本会先刷新该节点快照,再校准日报里的周期流量。日报会显示 `距重置 X天` 与 `重置周期内已用流量`。

推送样例:

```
📊 VolMonitor 日报  2026-06-18
节点 2 · 在线 1 · 离线 1 · 失败 287次
...
————————————
🟢 香港家宽
  今日  ↓3.73G · ↑1.40G · ↓↑5.13G
  周期  ↓12.00G · ↑5.00G · ↓↑17.00G
  状态  失败 0次 · 距重置 12天

🔴 死机
  今日  ↓0.00G · ↑0.00G · ↓↑0.00G
  周期  ↓0.50G · ↑0.20G · ↓↑0.70G
  状态  失败 287次 · 距重置 12天
```
...
- `VolMon.sh` v1.4.22:节点管理里支持按列表序号或节点名称选择目标节点,重置 / 修改周期已用 / 删除 / 重命名等操作都可直接输入序号。
- `VolMon.sh` v1.4.21:修复日报周期流量使用今日基线的问题;重置时写入周期基线并保持每日基线日期正确;节点匹配改为精确匹配;Telegram 文本转义;校准周期已用前刷新节点快照。
- `VolMon.sh` v1.4.20:节点管理新增修改当前周期已用流量,日报、状态总览与实时拉取的流量展示补充上下行合计。
- `VolMon.sh` v1.4.19:过了固定重置日后自动滚入新周期,同步刷新周期基线和下次重置日期。
- `VolMon.sh` v1.4.18:修正距重置天数按日期差计算,避免当天过了零点后少算一天。
- `VolMon.sh` v1.4.17:优化每日日报排版,总览与节点详情拆成多行显示,提升可读性。
- `VolMon.sh` v1.4.16:每月固定重置日遇到当月不存在的日期时顺延到下月 1 日;每日日报菜单支持按 `r` 立即发送。
